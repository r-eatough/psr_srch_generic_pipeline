...
$SCRIPT_DIR/symlink_cpy_psrfits.sh $target $OBS_DATE $BEAM $PROC_DIR $DATA_DIR

# dedisperse (1). dedisp split in four chunks due to file open limit!
prlimit -n4096 $SCRIPT_DIR/dedisperse_all_psrfits.sh $target $OBS_DATE $BEAM $PROC_DIR $SCRIPT_DIR

# dedisperse (2). dedisp split in four chunks due to file open limit! 
prlimit -n4096 $SCRIPT_DIR/dedisperse_all_psrfits_Prime.sh $target $OBS_DATE $BEAM $PROC_DIR $SCRIPT_DIR

# UNUSED DEDISPERSION TRIALS (add or remove as needed).
# dedisperse (3). dedisp split in four chunks due to file open limit! 
#prlimit -n4096 $SCRIPT_DIR/dedisperse_all_psrfits_PrimePrime.sh $target $OBS_DATE $BEAM $PROC_DIR $SCRIPT_DIR

# dedisperse (4). dedisp split in four chunks due to file open limit! 
#prlimit -n4096 $SCRIPT_DIR/dedisperse_all_psrfits_PrimePrimePrime.sh $target $OBS_DATE $BEAM $PROC_DIR $SCRIPT_DIR

# fft and acceleration search (Calling new conatainer -- psr_tools2.sif -- with parallel installed!)
prlimit -n4096 $SCRIPT_DIR/accelsearch_all_psrfits.sh $target $OBS_DATE $BEAM $PROC_DIR $SCRIPT_DIR

# sift candidates
$SCRIPT_DIR/sift_all_psrfits.sh $target $OBS_DATE $BEAM $PROC_DIR $SCRIPT_DIR

# fold candidates
prlimit -n4096 $SCRIPT_DIR/fold_all_psrfits.sh $target $OBS_DATE $BEAM $PROC_DIR $SCRIPT_DIR $RESULTS_DIR
